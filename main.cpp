...
private:
    ...
    int ChooseFlight(const vector<Flight>& flights) const {
        ...
        return clsInputValidate::ReadIntNumberBetween(1, flights.size());
    }
    ...
    int ChooseRoom(const vector<HotelRoom>& rooms) const {
        ...
        return clsInputValidate::ReadIntNumberBetween(1, rooms.size());
    }
...
public:
    ...
    void loaddatabase() {
        ...
        customrUser->SetUserName("Gad");
        customrUser->SetPassword("1470");
        customrUser->SetEmail("ziad.zezo881@gmail.com");
        customrUser->SetName("Gad Amr Gadelhaq");
    ...
    void AccessSystem() {
        int choice = readmenu({ "Login", "customer Sign Up", "Exit" });
        if (choice == 1) login();
